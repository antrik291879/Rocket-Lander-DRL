...
class RocketLander(gym.Env):
    ...
    def __init__(self, settings):
        ...
        self.impulsePos = (0, 0)
                ...
                # Continuous action space: [main_engine, side_engine, nozzle_angle]
        # main_engine: 0 → 1 (throttle)
        # side_engine: -1 → 1 (left/right thrust)
        # nozzle_angle: -1 → 1 (control nozzle direction)
    ...
    def reset(self, *, seed=None, options=None):
        ...
        obs = self._reset()   # call the original reset logic
    ...
    def step(self, action):
        ...
        # Gymnasium wants (obs, reward, terminated, truncated, info)
        terminated = done
        truncated = False
        return np.array(obs, dtype=np.float32), reward, terminated, info
    ...
    def refresh(self, mode='human', render=False):
        ...
        if render:
            self.render()
        return self.viewer.render(return_rgb_array=mode == 'rgb_array') if hasattr(self.viewer, 'render') else None
